...
class RegisterSensorResponse:
    def __init__(self, filter, pgdb):
        ...
        prc_name = None
        sqlId = "SELECT name_prc FROM %s.procedures" % (
        ...
        if prc_name:
            raise Exception(
                "Procedure '%s' already exist, consider to change the name" % (
                    prc_name))
        ...
        try:
            ...
            sqlId = "SELECT id_fty FROM %s.feature_type" % (
                filter.sosConfig.schema)
            ...
            try:
                ...
                sqlIns = "INSERT INTO %s.feature_type (name_fty)" % (
                    filter.sosConfig.schema)
                sqlIns += " VALUES (%s) RETURNING id_fty"
                params = (str(filter.foiType),)
                try:
                    id_fty = pgdb.executeInTransaction(
                        sqlIns, params)[0]["id_fty"]
                except:
                    raise Exception("SQL: %s" % (sqlIns))

            sqlIns = (
                "INSERT INTO %s.foi (name_foi, desc_foi, "
                "geom_foi, id_fty_fk)"
            ) % (filter.sosConfig.schema)
            sqlIns += (
                " VALUES (%s, %s, st_transform("
                "ST_GeomFromGML(%s),%s),%s) RETURNING id_foi"
            )
            params = (
                str(filter.foiName),
                str(filter.foiDesc),
            ...
            if filter.systemType in [
                ...
                if not par.split(":")[-1] == "iso8601":
                    if par in filter.oprDef:
                        ...
                        raise Exception(
                            "Field %s not found in Components %s" % (
                                par, filter.oprDef))
                    sqlId = "SELECT id_opr FROM %s.observed_properties" % (
                        filter.sosConfig.schema)
                    ...
                    try:
                        ...
                        sqlIns = (
                            "INSERT INTO %s.observed_properties "
                            "(name_opr,def_opr)"
                        ) % (filter.sosConfig.schema)
                        sqlIns += " VALUES (%s, %s) RETURNING id_opr"
                        params = (str(oprName), str(oprDef))
                        try:
                            id_opr = pgdb.executeInTransaction(
                                sqlIns, params)[0]["id_opr"]
                            com = True
                            opr_ids.append(id_opr)
                        except:
                            raise Exception("SQL: %s" % (
                                pgdb.mogrify(sqlIns, params)))

            elif filter.systemType == 'insitu-mobile-point':
                oty = 'insitu-mobile-point'
                if not (
                        par.split(":")[-1] in filter.sosConfig.parGeom["x"] or
                        par.split(":")[-1] in filter.sosConfig.parGeom["y"] or
                        par.split(":")[-1] in filter.sosConfig.parGeom["z"] or
                        par.split(":")[-1] == "iso8601"):
        ...
        try:
            ...
            sqlIns = "INSERT INTO %s.obs_type (name_oty,desc_oty)" % (
                filter.sosConfig.schema)
            sqlIns += " VALUES (%s,%s) RETURNING id_oty"
            params = (str(oty), None)
            try:
                id_oty = pgdb.executeInTransaction(
                    sqlIns, params)[0]["id_oty"]
                com = True
            except:
                raise Exception("SQL: %s" % (sqlIns))
            ...
            if oty in ['insitu-fixed-point','insitu-fixed-specimen','virtual', 'virtual-profile', 'profile']:
                if not par.split(":")[-1] == "iso8601":
                    ...
                    try:
                        ...
                        sqlIns = "INSERT INTO %s.uoms (name_uom,desc_uom)" % (
                            filter.sosConfig.schema)
                        sqlIns += " VALUES (%s,%s) RETURNING id_uom"
                        params = (uom, None)
                        try:
                            id_uom = pgdb.executeInTransaction(
                                sqlIns, params)[0]["id_uom"]
                            com = True
                            uom_ids.append(id_uom)
                        except:
                            raise Exception("SQL: %s" % (
        ...
        sqlId = "SELECT id_off FROM %s.offerings WHERE" % (
            filter.sosConfig.schema)
        sqlId += " name_off='temporary'"
        try:
            id_off = pgdb.select(sqlId)[0]["id_off"]
        except:
            sqlIns = "INSERT INTO %s.offerings (name_off,desc_off) VALUES" % (
                filter.sosConfig.schema)
            sqlIns += (
                " ('temporary','temporary offering to hold self-registered "
                "procedures/sensors waiting for service adimistration "
                "acceptance') RETURNING id_off"
            )
            try:
        ...
        #--insert procedure
        sqlIns = "INSERT INTO %s.procedures (id_foi_fk, id_oty_fk, " % (
            filter.sosConfig.schema)
        ...
        if not filter.beginPosition == 'NULL':
            ...
            params.extend([
                str(filter.beginPosition),
                str(filter.beginPosition)
            ])
        else:
            sqlIns += "%s , %s, "
            params.extend([None, None])
        # (select(md5(current_timestamp::text))))" >> removed because
        # of conflicts with pgpool replication
        sqlIns += " %s, %s, %s )"
        params.append(filter.time_sam_val)
        params.append(filter.time_acq_val)
        # Creating unique id python side avoiding pgpool replication
        # conflict (GSOC 2015)
        ...
        #--link proc_obs
        sqlIns = (
            "INSERT INTO %s.proc_obs (id_prc_fk, id_uom_fk, id_opr_fk, "
            "constr_pro) VALUES "
        ) % filter.sosConfig.schema
        ...
        #--link off_prc
        sqlIns = "INSERT INTO %s.off_proc (id_off_fk, id_prc_fk)" % (
            filter.sosConfig.schema)
        ...
        self.assignedSensorId = (
            filter.sosConfig.urn["sensor"] + ret_prc["assignedid_prc"]
        )
        ...
        if filter.sosConfig.transactional_log in ['True', 'true', 1]:
            sqlLog = (
                "INSERT INTO %s.tran_log (operation_trl,procedure_trl)"
            ) % filter.sosConfig.schema
